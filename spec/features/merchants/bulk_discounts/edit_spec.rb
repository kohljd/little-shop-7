...
RSpec.describe "Merchant Bulk Discount Edit", type: :feature do
  describe "when merchant visits" do
    ...
    before do
      visit edit_merchant_bulk_discount(merchant_1, bulk_discount_1)
    end

    it "displays update form with bulk discount's current info" do
      expect(page).to have_field("Discount", value: 20)
      expect(page).to have_field("Quantity", value: 10)
    end
    ...
    it "updated form redirects to Bulk Discount Show" do
      ...
      expect(page).to have_field("Discount", value: 25)
      expect(page).to have_field("Quantity", value: 30)
